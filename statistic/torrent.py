from statistic.torrent_statistics import *
...
class torrent():
    def __init__(self, torrent_metadata, client_request):
        ...
        self.staistics = torrent_statistic(self.torrent_metadata)
        self.block_length = 16*(2**10)
        self.piece_length = torrent_metadata.piece_length

        if self.client_request['seeding'] != None:
            self.staistics.num_pieces_downloaded = self.torrent_metadata.file_size
    ...
    def get_piece_length(self, piece_index):
        if piece_index == self.pieces_count-1:
            return self.torrent_metadata - self.torrent_metadata.piece_length*piece_index
        else:
